...
class lstm_model():
    ...
    def __init__(self, FLAGS, initializer=
        ...
        #self.intent_size = FLAGS.intent_size
        self.batch_size = FLAGS.batch_size
        self.learning_rate = FLAGS.learning_rate
        self.initializer = initializer
        #self.max_time = FLAGS.max_time
        #self.lamda = FLAGS.lamda
        self.s_cnum = FLAGS.s_cnum
        #self.u_cnum = FLAGS.u_cnum
        #self.sample_num = FLAGS.sample_num
        self.margin = FLAGS.margin
        #self.lstm_num = FLAGS.lstm_num # bidirectional lstm # bidirectional lstm
        ...
        self.word_mask = tf.placeholder("float", [None, self.n])
        self.s_len = tf.placeholder("int64", [None])
        """
        self.input_y = tf.placeholder("float", [None, self.intent_size]) 
        self.input_y_id = tf.placeholder("float", [None])
        self.label_vec = tf.placeholder(tf.float32, 
                shape=[None, self.intent_size])
        """
        self.IND = tf.placeholder(tf.float32, shape=[None, self.s_cnum])

        # graph
        self.instantiate_weights()
        self.attention, self.hidden_states, self.sentence_embedding = self.inference()
        #self.prediction = self.predict()
        #self.accuracy = self.evaluate()
        # capsule
        self.activation, self.weights_c, self.votes = self.capsule()
    ...
    def instantiate_weights(self):
        ...
        """
        with tf.name_scope("capsule_bias"):
            self.capsule_biases = tf.get_variable("capsule_bias",
                    shape=[self.s_cnum, self.output_atoms])
        """
    ...
    def _update_routing(self, votes, logit_shape, num_dims, input_dim, output_dim):
        ...
        _, logits, activations = tf.while_loop(
            lambda i, logits, activations: i < self.num_routing,
            _body,
            loop_vars=[i, logits, activations],
            swap_memory=True)

        return activations.read(self.num_routing - 1), logits
    ...
    def capsule(self):
        ...
        input_tiled = tf.tile(tf.expand_dims(self.sentence_embedding, -1),
        ...
        activations, weights_c = self._update_routing(
        ...
        return activations, weights_c, votes_reshaped
    ...
    def loss(self):
        #classification_loss = tf.nn.sigmoid_cross_entropy_with_logits(self.IND, logits)
        loss_val = self._margin_loss(self.IND, self.logits)
        loss_val = tf.reduce_mean(loss_val)


        # self attention regularizer
        ...
        #return self.alpha * tf.reduce_mean(self_atten_loss)
        #return 1000 * loss_val
        return 1000 * loss_val + self.alpha * tf.reduce_mean(self_atten_loss)

        """
        # set non-word attention to zero
        # [N, 11] => [N, 1, 11] => [N, 8, 11]
        word_masks = tf.tile(tf.expand_dims(self.word_mask, 1), [1, self.r, 1])
        att = tf.multiply(self.attention, word_masks)
        norm_att = tf.nn.l2_normalize(att, axis=-1)
        norm_att += (1 - word_masks) * 1e-20
        """

        """
        # self attention KL regularizer
        # original shape: [64, 8, 11] => [8, 64, 11]
        attn = tf.transpose(norm_att, perm = [1, 0, 2])
        self_atten_loss = tf.map_fn(lambda x: tf.map_fn(
            lambda y: tf.distributions.kl_divergence(
                tf.contrib.distributions.Categorical(probs=x),
                tf.contrib.distributions.Categorical(probs=y)), attn, parallel_iterations = 100), attn, parallel_iterations = 100)
        #return self.alpha * tf.reduce_sum(self_atten_loss) / (self.r * (self.r - 1))
        #return 10 * loss_val
        return 5000 * loss_val - self.alpha * tf.reduce_sum(self_atten_loss) / (self.r * (self.r - 1))
        """
